...
def write_fragmentation_reports(df: pd.DataFrame, out_csv: str, out_md: str) -> Dict[str, int]:
    ...
    if df.empty:
        ...
        lines.append("")
        lines.append(over_100.to_markdown(index=False) if len(over_100) else "_Aucun_")
        lines.append("")
        lines.append(f"## Agents multi-équipes ({len(multi_team)})")
        lines.append("")
        lines.append(multi_team.to_markdown(index=False) if len(multi_team) else "_Aucun_")
        lines.append("")
