...
def compute_fragmentation(data, mapping: Dict) -> pd.DataFrame:
    ...
    aff["_person_id"] = aff[aff_person].apply(parse_ref_id)
    aff["_agent"] = aff["_person_id"].map(person_map).fillna("UNKNOWN").astype(str)
    aff["_team"] = aff[aff_team].apply(parse_ref_id)
    aff["_epic"] = aff[aff_epic].apply(parse_ref_id) if aff_epic in aff.columns else None
