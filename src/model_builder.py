...
def build_model(
    ...
    if not affectations.empty:
        ...
        for tid, grp in affectations.dropna(subset=["_team_id"]).groupby("_team_id"):
            ...
            grp_members = grp_pos.loc[~grp_pos["_role"].isin([pm_role, po_role])]
            team_people[int(tid)] = set(grp_members["_person_label"].tolist())
    ...
    # Deduplicate while preserving order (per team).
